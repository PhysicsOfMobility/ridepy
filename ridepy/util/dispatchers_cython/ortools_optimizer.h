...
void optimize_stoplists(vector<vector<Stop<Loc>>> stoplists,
  ...
  // We will create the data structures needed to initialize the ortools routing problem
  // A flat list of all stops
  ...
  for (auto &stoplist : stoplists) {
    is_cpe_stop = true;
    onboard_requests_dropoff_idxs.push_back(vector<int>(0));
  ...
  for (int i = 1; i < all_stops.size(); ++i) {
    if (i==end_loc_idx or (binary_search(start_loc_idxs.begin(), start_loc_idxs.end(), i))) continue;
  ...
  for (int vehicle_idx = 0; vehicle_idx < stoplists.size(); ++vehicle_idx) {
    int64_t index = routing.Start(vehicle_idx);
    auto start_loc_idx = start_loc_idxs[vehicle_idx];
    time_dimension.CumulVar(index)->SetRange(time_windows[start_loc_idx].first,
                                             time_windows[start_loc_idx].second);
  }

  // Force the dropoffs of the onboard requests to be served by the correct vehicle.
  vehicle_idx = 0;
  for (auto onboard_requests_dropoff_idxs_one_vehicle=onboard_requests_dropoff_idxs.begin();
       onboard_requests_dropoff_idxs_one_vehicle!=onboard_requests_dropoff_idxs.end();
       ++onboard_requests_dropoff_idxs_one_vehicle, ++vehicle_idx)
  {
    for (auto &onboard_request_dropoff_idx: *onboard_requests_dropoff_idxs_one_vehicle)
    {
    int64_t index = manager.NodeToIndex(RoutingIndexManager::NodeIndex(onboard_request_dropoff_idx));
