...
cdef class VehicleState:
    ...
    def select_new_stoplist(self):
        if self.loc_type == LocType.R2LOC:
            ...
            dereference(self._uvstate.vstate_int).stoplist = self._ustoplist_tentative.c_stoplist_int
        else:
            raise ValueError("This line should never have been reached")
    ...
    property stoplist:
        def __get__(self):
            if self.loc_type == LocType.R2LOC:
                ...
                return Stoplist.from_c_int(dereference(self._uvstate.vstate_int).stoplist)
            else:
                raise ValueError("This line should never have been reached")
