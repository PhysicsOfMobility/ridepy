...
int main(int, char*[])
{
    ...
    const std::vector<Edge> edges{
        Edge(101, 102), Edge(102, 103),
        Edge(103, 104), Edge(104, 101),
        Edge(101, 103)
    };
    ...
    for (auto& src: vertices){
        for (auto& target: vertices){
            std::cout << "d(" << src << "," << target << "): " << g.distance(src, target) << std::endl;
        }
    }
    for (auto dtd=0; dtd<18; dtd++){
        auto [v, rest] = g.interpolate(102, 104, dtd);
        std::cout << "interpolate(102, 104, " << dtd << "): " << v << "," << rest << std::endl;
    }
}