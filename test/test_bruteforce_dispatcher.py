...
def test_append_to_empty_stoplist(kind):
    ...
    if kind == "cython":
        (new_new_stoplist,) = optimize_stoplists(
            [new_stoplist], space, [10], current_time=0, time_resolution=1e-8
        )
...
def test_timewindow_violation_at_dropoff_checked(kind):
    ...
    due to dropoff+pickup together would violate a tw_max. Although separately, these
    delays do not.
    """
    # fmt: off
    # location, cpat, tw_min, tw_max, occupancy
    ...
    # but the above should not have occurred if the tw_max at the last stop were higher:
    # fmt: off
    stoplist_properties = [
        [(0, 1), 1, 0, inf],
        [(0, 2), 2, 2, 4],
...
def test_inserted_at_the_middle(kind):
    ...
    if kind == "cython":
        (new_new_stoplist,) = optimize_stoplists(
            [new_stoplist], space, [10], current_time=0, time_resolution=1e-10
        )
        assert new_stoplist == new_new_stoplist
    assert new_stoplist[1].location == request.origin
