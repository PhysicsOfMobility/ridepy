...
@pytest.fixture
def r1():
    return TransportationRequest(request_id=7, creation_timestamp=1.8, origin=(3,7), destination=(2,1),
                               pickup_timewindow_min=2.13, pickup_timewindow_max=inf, delivery_timewindow_min=4.24,
                               delivery_timewindow_max=inf)
@pytest.fixture
def r2():
    return TransportationRequest(request_id=8, creation_timestamp=2, origin=(4,1), destination=(3,7),
                               pickup_timewindow_min=0, pickup_timewindow_max=inf, delivery_timewindow_min=0,
                               delivery_timewindow_max=inf)
@pytest.fixture
def s0(r0):
    return Stop(location=r0.location, request=r0, action=StopAction.internal, estimated_arrival_time=3.67, time_window_min=9.12, time_window_max=inf)
@pytest.fixture
def s1(r1):
    return Stop(location=r1.origin, request=r1, action=StopAction.pickup, estimated_arrival_time=2.51, time_window_min=inf, time_window_max=9.13)
@pytest.fixture
def s2(r2):
    return Stop(location=r2.origin, request=r2, action=StopAction.pickup, estimated_arrival_time=3.86, time_window_min=1, time_window_max=inf)
@pytest.fixture
def s3(r1):
    return Stop(location=r1.destination, request=r1, action=StopAction.dropoff, estimated_arrival_time=2.17, time_window_min=1, time_window_max=inf)
@pytest.fixture
def s4(r2):
    return Stop(location=r2.destination, request=r2, action=StopAction.dropoff, estimated_arrival_time=7.16, time_window_min=1, time_window_max=inf)
@pytest.fixture
...
def test_attr_get():
    ...
    r1 = TransportationRequest(request_id=7, creation_timestamp=1.8, origin=(3,7), destination=(2,1),
                               pickup_timewindow_min=2.13, pickup_timewindow_max=inf, delivery_timewindow_min=4.24,
                               delivery_timewindow_max=inf)
    ...
    s0 = Stop(location=r0.location, request=r0, action=StopAction.internal, estimated_arrival_time=3.67, time_window_min=9.12, time_window_max=inf)
    ...
    s1 = Stop(location=r1.origin, request=r1, action=StopAction.pickup, estimated_arrival_time=2.51, time_window_min=inf, time_window_max=9.13)
...
def test_attr_get_stoplist(r0, r1, r2, s0, s1, s2, s3, s4, stoplist):
    ...
    assert_array_almost_equal([s.request.request_id for s in stoplist], [999, 7, 8, 7, 8])
    assert_array_almost_equal([s.request.creation_timestamp for s in stoplist], [7.89, 1.8, 2, 1.8, 2])
    assert_array_almost_equal([s.estimated_arrival_time for s in stoplist], [3.67, 2.51, 3.86, 2.17, 7.16])
...
@pytest.mark.xfail(reason="Stoplist.__getitem__ returns a wrapper around &self.ustoplist._stoplist_r2loc[i]")
