...
private:
    Graph _g;
    std::map<vertex_t, int> vertex_label2index;
    ...
    std::vector<int> _distances, _predecessors;
    std::vector<double> _weights;

public:
    GraphSpace(std::vector<vertex_t> vertex_vec, std::vector<Edge> edge_vec)
        : GraphSpace(vertex_vec, edge_vec, std::vector<double> {edge_vec.size(), 1}){}

    GraphSpace(std::vector<vertex_t> vertex_vec, std::vector<Edge> edge_vec, std::vector<double> weight_vec)
        : _g{vertex_vec.size()}, vertex2label{get(vertex_name, _g)},
    ...
    double distance(vertex_t src, vertex_t target){
        // call dijkstra
    ...
    std::pair<vertex_t, double> interpolate(vertex_t u, vertex_t v, double dist_to_dest) {
        /*
        def interp_dist(self, u, v, dist_to_dest):
            ...
            current_node = v
            while current_node is not u:
                predecessor = self._predecessors[u][current_node]
                dist_from_dest = self.d(predecessor, v)
                if dist_from_dest >= dist_to_dest:
                    break
                current_node = predecessor

            if dist_from_dest > dist_to_dest:
                return current_node, dist_to_dest - self.d(current_node, v)
            else:
                return predecessor, 0
        */
        if (u == v) return std::make_pair(v, 0);
        // call dijkstra
        int u_idx = this->vertex_label2index[u];
        int v_idx = this->vertex_label2index[v];
        ...
        if (dist_from_dest > dist_to_dest) return std::make_pair(this->vertex2label[current_node], dist_to_dest
            - dist_from_dest + current_edge_weight);
        else return std::make_pair(this->vertex2label[predecessor], 0);
    }
    ...
    void print_shortest_paths(vertex_t src){
         ...
         std::cout << "Shortest paths from node " << this->vertex2label[src_idx] << ":" << std::endl;
         for (auto vp = vertices(this->_g); vp.first != vp.second; ++vp.first){
             int vertex = *(vp.first);
             int vertex_label = this->vertex2label[vertex];
             std::cout << "to " << vertex_label << ": " << this->_distances[vertex] << std::endl;
             std::cout<<"Path: ";
             int j=vertex;
             while (j != src_idx){
                 std::cout << this->vertex2label[j] <<", ";
                 j = this->_predecessors[j];
             }
             std::cout<< this->vertex2label[src_idx] <<std::endl;
         }
     }
    ...
    void print_vertices_and_edges(){
        // print the vertices
        for (auto vp = vertices(this->_g); vp.first != vp.second; ++vp.first){
            std::cout << "vertex: " << *(vp.first) << ", label:" << this->vertex2label[*(vp.first)] << std::endl;
        }
        // print the edge_vec
        for (auto [first, last] = edges(this->_g); first != last; ++first){
            std::cout << "Edge(" << this->vertex2label[source(*first, this->_g)] << ","
                << this->vertex2label[target(*first, this->_g)] << ")" << std::endl;
        }
    };
