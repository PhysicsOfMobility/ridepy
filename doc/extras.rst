...
.. currentmodule:: thesimulator.extras.simulate

This module allows to configure and execute a set of set of simulations
...
.. code-block:: python
    ...
    # execute simulations
    simulate(data_dir=output_path, conf=conf)

Parameter Scan Configuration
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For `.simulate()` to work, all parameters in the base parameter scan configuration supplied by `.get_default_conf()`
...
When `.simulate()` is called, independent simulations are performed for each parameter set drawn from
...
parameter scan are returned by `.simulate()`.
...
.. automodule:: thesimulator.extras.simulate
    :members:
